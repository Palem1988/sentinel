...
def has_anon_conf():
    ...
    #
    # if not, print a message stating that Anon Core must be installed and
    # configured, including JSONRPC access in anon.conf
    try:
...
def main():
    ...
    if not has_anon_conf():
        print("AnonCore must be installed and configured, including JSONRPC access in anon.conf")
        sys.exit(1)
